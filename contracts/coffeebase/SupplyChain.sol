...
import "../coffeaccesscontrol/ConsumerRole.sol";
import "../coffeaccesscontrol/DistributorRole.sol";
import "../coffeaccesscontrol/RetailerRole.sol";
import "../coffeaccesscontrol/FarmerRole.sol";
...
contract SupplyChain is RetailerRole, FarmerRole, DistributorRole, ConsumerRole  {
    ...
    modifier paidEnough(uint256 _price) {
        require(msg.value >= _price,uint2str(msg.value));
        _;
    }
    ...
    modifier harvested(uint256 _upc) {
        require(items[_upc].itemState == State.Harvested, 'Status not Harvested');
        _;
    }

    // Define a modifier that checks if an item.state of a upc is Processed
    modifier processed(uint256 _upc) {
        require(items[_upc].itemState == State.Processed, 'Status not Processed');
        _;
    }

    // Define a modifier that checks if an item.state of a upc is Packed
    modifier packed(uint256 _upc) {
        require(items[_upc].itemState == State.Packed, 'Status not Packed');
        _;
    }

    // Define a modifier that checks if an item.state of a upc is ForSale
    modifier forSale(uint256 _upc) {
        require(items[_upc].itemState == State.ForSale, 'Status not ForSale');
        _;
    }

    // Define a modifier that checks if an item.state of a upc is Sold
    modifier sold(uint256 _upc) {
        require(items[_upc].itemState == State.Sold, 'Status not Sold');
        _;
    }

    // Define a modifier that checks if an item.state of a upc is Shipped
    modifier shipped(uint256 _upc) {
        require(items[_upc].itemState == State.Shipped, 'Status not Shipped');
        _;
    }

    // Define a modifier that checks if an item.state of a upc is Received
    modifier received(uint256 _upc) {
        require(items[_upc].itemState == State.Received, 'Status not Received');
        _;
    }

    // Define a modifier that checks if an item.state of a upc is Purchased
    modifier purchased(uint256 _upc) {
        require(items[_upc].itemState == State.Purchased, 'Status not Purchased');
        _;
    }
